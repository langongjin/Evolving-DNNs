...
def render_nn(nn, ax=None, 
    ...
    for connection in nn.connections:
        ...
        if connection.weight > 0.0:
            ax.arrow(n1_x, n1_y, offsetx+ox, offsety+oy, head_width = node_radius*2,
                     head_length = node_radius*1.2, fc='red', ec='red', length_includes_head=False, #1.2 node_radius red
                     linewidth = 0.5*abs(wg), 
                     alpha = connection_alpha*np.clip(0.1+abs(connection.weight)/max_weight, 0, 1))
        else:
            ax.arrow(n1_x, n1_y, offsetx+ox, offsety+oy, head_width = node_radius*2,
                     head_length = node_radius*1.2, fc='blue', ec='blue', length_includes_head=False,
                     linewidth = 0.5*abs(wg), 
                     alpha = connection_alpha*np.clip(0.1+abs(connection.weight)/max_weight, 0, 1))
    ...
    for index in range(len(nn.neurons)):
        ...
        if n.type == NEAT.NeuronType.INPUT:
            ax.add_patch(plt.Circle((nx, ny), node_radius, ec='green', fc=(0.327,0.327,0.327), linewidth=1, zorder=2)) #3 linewidht
        elif n.type == NEAT.NeuronType.BIAS:
            ax.add_patch(plt.Circle((nx, ny), node_radius, ec='black', fc=(1,1,1), linewidth=1, zorder=2))
        elif n.type == NEAT.NeuronType.HIDDEN:
            ax.add_patch(plt.Circle((nx, ny), node_radius, ec='black', fc=(0.127,0.127,0.127), linewidth=1, zorder=2))
        elif n.type == NEAT.NeuronType.OUTPUT:
            ax.add_patch(plt.Circle((nx, ny), node_radius, ec='brown', fc=(0,1,0), linewidth=1, zorder=2))
...
def plot_nn(nn, ax=None, 
    ...
    if type(nn) == NEAT.Genome:
        kk = NEAT.NeuralNetwork()
...
#     def DrawPhenotype(image, rect, nn, neuron_radius=7,
#                       max_line_thickness=0.01, substrate=False):
#         for i, n in enumerate(nn.neurons):
#             nn.neurons[i].x = 0
#             nn.neurons[i].y = 0
...
#                 neuron.x = NEAT.Scale(neuron.substrate_coords[0], min_x, max_x,
#                                  rect_x_size / 15,
#                                  rect_x_size - rect_x_size / 15)
#                 neuron.y = NEAT.Scale(neuron.substrate_coords[1], min_y, max_y,
#                                  rect_x_size / 15,
...
#             thickness = NEAT.Scale(thickness, 0, max_weight, 1, max_line_thickness)
#             thickness = NEAT.Clamp(thickness, 1, max_line_thickness)

#             w = NEAT.Scale(abs(conn.weight), 0.0, max_weight, 0.0, 1.0)
#             w = NEAT.Clamp(w, 0.75, 1.0)
...
#             else:
#                 # Draw a line
#                 pt1 = (int(nn.neurons[conn.source_neuron_idx].x + 100),
#                        int(nn.neurons[conn.source_neuron_idx].y+100))
#                 pt2 = (int(nn.neurons[conn.target_neuron_idx].x+80),
#                        int(nn.neurons[conn.target_neuron_idx].y-50))
#                 cv2.line(image, pt1, pt2, color, int(thickness))

#          # draw all neurons
#         for neuron in nn.neurons:
#             pt = (int(neuron.x)+100, int(neuron.y)+100)
#             pt2 = (int(neuron.x)+80, int(neuron.y)-50)
...
#             a = NEAT.Clamp(a, 0.3, 2.0)

#             if np.isnan(a):
#                 a = 1.0

#             if neuron.type == NEAT.NeuronType.INPUT:
#                 cv2.circle(image, pt, int(neuron_radius*a), clr, thickness=-3) # filled 
#                 cv2.circle(image, pt, neuron_radius, (0,255,0), thickness=3) # outline
#             elif neuron.type == NEAT.NeuronType.BIAS:
#                 cv2.circle(image, pt, int(neuron_radius*a), clr, thickness=-3) # filled 
#                 cv2.circle(image, pt, neuron_radius, (0,0,0), thickness=3) # outline 
#             elif neuron.type == NEAT.NeuronType.HIDDEN:
#                 cv2.circle(image, pt, int(neuron_radius*a), (0,0,0), thickness=10) # filled 
#                 cv2.circle(image, pt, neuron_radius, (0,0,0), thickness=3) # outline (127,127,127)
#             elif neuron.type == NEAT.NeuronType.OUTPUT:
#                 cv2.circle(image, pt2, int(neuron_radius*a), clr, thickness=-3) # filled first
#                 cv2.circle(image, pt2, neuron_radius, (127,127,127), thickness=3) # outline 
            
            

# More general one that returns a NumPy image
def Draw(x): #, size=(800,800)): #300
    # img = np.zeros((size[0], size[1], 3), dtype=np.uint8)
    # img += 255

    # if isinstance(x, NEAT.NeuralNetwork):
    #     DrawPhenotype(img, (0, 0, 700, 700), x ) #250
    # else:
    #     nn = NEAT.NeuralNetwork()
    #     x.BuildPhenotype(nn)
        # DrawPhenotype(img, (0, 0, 700, 700), nn )
    # nn = NEAT.NeuralNetwork()
    # x.BuildPhenotype(nn)
    # plot_nn(x, ax=None, 
    #         is_substrate=False, 
    ...
    # image.savefig('test_pplt.pdf', dpi=100)
